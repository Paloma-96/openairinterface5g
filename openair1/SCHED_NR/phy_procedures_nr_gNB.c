...
void fill_ul_rb_mask(PHY_VARS_gNB *gNB, int frame_rx, int slot_rx) {
  ...
  for (int i = 0; i < gNB->max_nb_srs; i++) {
    ...
    if (srs) {
      ...
      // print the rb_mask_ul showing where the bit is set to 1 and where it is set to 0 knowing that uint32_t rb_mask_ul[14][9]
      /*
      
      ...
      printf("[PALOMA HACK] gNB->rb_mask_ul\n");
      for (int symbol = 0; symbol < gNB->frame_parms.symbols_per_slot; symbol++) {
        for (int rb = 0; rb < gNB->frame_parms.N_RB_UL; rb++) {
          if (gNB->rb_mask_ul[symbol][rb >> 5] & (1U << (rb & 31))) {
            printf("1");
          } else {
            printf("0");
          }
...
int phy_procedures_gNB_uespec_RX(PHY_VARS_gNB *gNB, int frame_rx, int slot_rx)
{
  ...
  for (int i = 0; i < gNB->max_nb_srs; i++) {
    ...
    if (srs) {
      //if ((srs->active == 1) && (srs->frame == frame_rx) && (srs->slot == slot_rx)) {
      if ((first_frame_founded == 0) && ((slot_rx == 8) || (slot_rx == 9) || (slot_rx == 18))) {
        ...
        */
        ...
        if (srs_est >= 0) {
          ...
          //printf("[PALOMA HACK] index pre = %d\n", index);
        ...
        if (my_snr > 0  && srs_ul_toa_array[index] > 0){
          ...
          printf("[PALOMA HACK] srs_ul_toa_array[index] = %d, my_snr = %d\n", srs_ul_toa_array[index], my_snr);
          printf("----------------------------------------------------------------------------------\n");
      ...
      else if ((frame_rx == first_frame_founded + 4 * iteration) && ((slot_rx == 8) || (slot_rx == 9) || (slot_rx == 18))) {
        ...
        if (my_snr > 0  && srs_ul_toa_array[index] > 0){
          ...
          printf("[PALOMA HACK] SRS TOA = %d, my_snr = %d\n", srs_ul_toa_array[index], my_snr);
          iteration ++;
        }
        else{
